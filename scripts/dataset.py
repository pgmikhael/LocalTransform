...
class USPTODataset(object):
    def __init__(
        ...
        self.use_protein_sequence = args["use_protein_sequence"]
        self.sequences = pickle.load(open("%s/sequences.p" % args["data_dir"], "rb"))
        self.uniprot2sequences = pickle.load(
            open(
        ...
        if self.sep:
            ...
            self.dgraph_path = (
                "../data/saved_graphs/full_%s_dgraph_sep.pkl" % args["dataset"]
            )  # changed by numpy matrix for faster processing speed
        else:
            self.labels = [eval(t) for t in df["Labels_mix"]]
            self.dgraph_path = (
                "../data/saved_graphs/full_%s_dgraph_mix.pkl" % args["dataset"]
            )
    ...
    def _pre_process(self, mol_to_graph, node_featurizer, edge_featurizer):
        ...
        if not self.fgraphs_exist or not self.dgraphs_exist:
            for s1, s2 in tqdm(
                zip(self.reactants, self.reagents),
                total=len(self.reactants),
                desc="Building dgl graphs...",
            ):
    ...
    def __getitem__(self, item):
        ...
        if self.use_protein_sequence:
            sequence = self.uniprot2sequences[self.sequences[item]]
        else:
            sequence = "<unk>"
        return (
            self.reactants[item],
...
class USPTOTestDataset(object):
    def __init__(
        ...
        self.use_protein_sequence = args["use_protein_sequence"]
        self.sequences = pickle.load(
            open("%s/test_sequences.p" % args["data_dir"], "rb")
        )
        self.uniprot2sequences = pickle.load(
            open(
                "/Mounts/rbg-storage1/datasets/Enzymes/EnzymeMap/uniprot2sequence.p",
        ...
        if self.sep:
            self.dgraph_path = (
                "../data/saved_graphs/test_%s_dgraph_sep.pkl" % args["dataset"]
            )  # changed by numpy matrix for faster processing speed
        else:
            self.dgraph_path = (
                "../data/saved_graphs/test_%s_dgraph_mix.pkl" % args["dataset"]
            )
        self._pre_process(mol_to_graph, node_featurizer, edge_featurizer)
    ...
    def __getitem__(self, item):
        ...
        dgraph["v_bonds"], dgraph["r_bonds"] = dgraph["bonds"]
        if self.use_protein_sequence:
            uniprot_id = self.sequences[item]
            sequence = self.uniprot2sequences.get(uniprot_id, "<unk>")
        else:
            sequence = "<unk>"
