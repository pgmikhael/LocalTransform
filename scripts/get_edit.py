...
def logit2pred(out, top_num):
    ...
    output_rank = [r for r in output_rank if get_id_template(r, class_n)[1] != 0][
        :top_num
    ]
...
def get_bond_site(graph, etype):
    atom_pair_list = torch.transpose(
        graph.adjacency_matrix(etype=etype).coalesce().indices(), 0, 1
    ).numpy()
...
def combined_edit(virtual_bonds, real_bonds, pred_vi, pred_ri, pred_v, pred_r, top_num):
    ...
    pred_site_v = [
        (list(pooled_virtual_bonds[pred_site]), pred_temp)
        for pred_site, pred_temp in pred_site_v
    ]
    pred_site_r = [
        (list(pooled_real_bonds[pred_site]), pred_temp)
        for pred_site, pred_temp in pred_site_r
    ]
...
def write_edits(args, model, test_loader):
    ...
    with open(args["result_path"], "w") as f:
        f.write(
            "Test_id\tReactants\tReagents\t%s\n"
            % "\t".join(["Prediction %s" % (i + 1) for i in range(args["top_num"])])
        )
        with torch.no_grad():
            for batch_id, data in enumerate(test_loader):
                reactants, reagents, bg, adms, bonds_dicts, protein_sequences = data
                pred_VT, pred_RT, _, _, pred_VI, pred_RI, _ = predict(
                    args, model, bg, adms, bonds_dicts
                )
                ...
                for i, (reactant, reagent) in enumerate(zip(reactants, reagents)):
                    ...
                    virtual_bonds, real_bonds = (
                        bonds_dicts["virtual"][i].numpy(),
